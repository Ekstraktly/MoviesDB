...
class MoviesPopulator
  def self.populate_movies
    (1..5).each do |page|
      ...
      JSON.parse(response.read_body)['results'].select do |movie|
        ...
        puts movie['title'], movie['overview'], movie['release_date'].partition('-')[0], movie['vote_average']
      end
    end
    return
  end
  ...
  def self.populate_genres
    url = URI("https://api.themoviedb.org/3/genre/movie/list?language=en-US&api_key=e95ef8f8eb154d4d0fb1ba5d47cb66fc")
